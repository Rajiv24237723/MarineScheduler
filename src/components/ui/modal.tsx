...
export function Modal({ open, onClose, title, subtitle, children, width = 'max-w-2xl' }: {
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div onClick={e => e.stopPropagation()} className={`animate-scale-in relative ${width} w-full max-h-[88vh] overflow-auto rounded-md border border-border/80 bg-card shadow-2xl`}>
